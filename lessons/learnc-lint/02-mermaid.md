...
This Rust parser is deliberately a partial gate. Some diagrams it accepts may still fail in the browser's Mermaid parser; authors should inspect diagrams in the lesson UI. Lint separately warns when a flowchart or class-diagram statement starts with `subgraph` or `style`; both can be legitimate authoring choices.
