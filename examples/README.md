...
[`repository-lesson.json`](repository-lesson.json) demonstrates worktree files,
Git blobs, patch files, and a generated worktree diff. Those sources require a
specific commit and a deliberate uncommitted change, so the companion script
creates that state instead of assuming it already exists in this checkout.
